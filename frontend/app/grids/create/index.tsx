...
const CreateGrid = () => {
  ...
  const [{ data: createGridData }, createGrid] = useAxios<Grid, { name: string; width: number; height: number }>(
    {
      url: '/grid',
      method: 'POST',
    },
      ...
      <form className="flex flex-col items-center justify-center p-md gap-4" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-4 w-lg">
          ...
          <Input
            ...
            label="Number of boxes horizontally"
            type="number"
            min={1}
            max={7}
          />
          <Input
            ...
            label="Number of boxes vertically"
            type="number"
            min={1}
            max={7}
          />
