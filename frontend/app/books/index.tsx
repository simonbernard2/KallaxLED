...
const BooksPage = () => {
  ...
  const [{ loading: updateLoading, error: updateError }, updateBook] = useAxios({ method: 'PUT' }, { manual: true })
  const [{ loading: deleteLoading, error: deleteError }, deleteBook] = useAxios({ method: 'DELETE' }, { manual: true })
  ...
  return (
    <div className="flex flex-col gap-10 w-full max-w-4xl">
            ...
            <div key={book.id} className="border rounded px-3 py-2 flex flex-col gap-2">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex gap-2">
                  <Button onClick={() => startEdit(book)} disabled={updateLoading}>
                    Edit
                  </Button>
                  <Button onClick={() => handleDelete(book.id!)} color="red" disabled={deleteLoading}>
                    Delete
                  </Button>
                </div>
              </div>
      ...
      {(deleteError || updateError) && !editingId && <div className="text-sm text-red-500">Error updating books.</div>}
    </div>
  )
}
