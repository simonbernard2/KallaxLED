  ...
  return (
    <section className="grid gap-4 lg:grid-cols-3">
      {overviewCards.map(card => (
        <Link key={card.href} to={card.href} className="panel flex min-h-52 flex-col justify-between hover:bg-white/90">
          <div>
          ...
          <span className="mt-6 inline-flex items-center text-sm font-semibold text-[var(--forest)]">Open {card.title}</span>
        </Link>
      ))}
    </section>
  )
