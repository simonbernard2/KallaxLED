...
const overviewCards = [
  {
    title: 'Books',
    description:
      'Create shelf books, attach box locations, and link Conjuring Archive references for topic-aware search.',
    href: '/manage/books',
  },
  {
    title: 'Grid',
    ...
    <section className="grid gap-4 lg:grid-cols-3">
      {overviewCards.map(card => (
        <Link
          key={card.href}
          to={card.href}
          className="panel flex min-h-52 flex-col justify-between hover:bg-[var(--surface-hover)]"
        >
          <div>
          ...
          <span className="mt-6 inline-flex items-center text-sm font-semibold text-[var(--forest-ink)]">
            Open {card.title}
          </span>
        </Link>
      ))}
